// src/app/dashboard/loans/components/ViewUserLoanDetailModal.tsx
"use client";
...
import type { Loan, UserViewLoanDetailModalProps, InfoPillProps } from "@/types";
import { DollarSign, CalendarDays, Hash, Percent, CheckCircle, XCircle, Clock, FileText } from "lucide-react";
...
export function ViewUserLoanDetailModal({
  isOpen,
  onOpenChange,
  loan,
}: UserViewLoanDetailModalProps) {
  if (!loan) return null;
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Viewing details for your loan application ID: {loan.id}.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 py-4 max-h-[60vh] overflow-y-auto pr-2">
          ...
          <InfoPill icon={<DollarSign className="mr-2 h-4 w-4" />} label="Loan Amount" value={`${loan.currency || '$'}${loan.amount.toLocaleString()}`} />
          ...
          {loan.disbursedDate && (
            <InfoPill icon={<CalendarDays className="mr-2 h-4 w-4" />} label="Disbursed Date" value={formatDateDisplay(loan.disbursedDate)} />
          )}
          {loan.paidDate && (
            <InfoPill icon={<CalendarDays className="mr-2 h-4 w-4" />} label="Paid Date" value={formatDateDisplay(loan.paidDate)} />
          )}
